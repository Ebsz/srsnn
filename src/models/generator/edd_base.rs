//! Evolved Dual Dynamics.
//! Differs from the ETD model in that only two types of dynamics (inhibitory/excitatory)
//! are optimized
...
impl Generator for EDDModel {
    fn params(config: &ModelConfig<Self>, env: &Environment) -> ParameterSet {
        ...
        // Dynamical parameters for the two kinds of neuron: [2, 4]
        let d = Parameter::Matrix(Array::zeros((2, Self::N_DYNAMICAL_PARAMETERS-1)));

        ParameterSet {
            set: vec![t_cpm, input_t_cpm, d],
        }
    }
    ...
    {
        let (m1, m2, m3) = Self::parse_params(params, config);
        ...
        let (dynamics, itypes) = Self::dual_dynamics(labels.clone(), m3, config);
...
impl EDDModel {
    pub fn parse_params<'a>(ps: &'a ParameterSet, config: &'a ModelConfig<Self>)
        ...
        assert!(d.shape() == [2, Self::N_DYNAMICAL_PARAMETERS-1]);
    ...
    fn dual_dynamics(l: LabelFn, m3: &Array2<f32>, config: &ModelConfig<Self>) -> (NeuronSet, Vec<usize>) {
        let (td, itypes) = Self::type_dynamics(m3, config);
    ...
    fn type_dynamics(m3: &Array2<f32>, config: &ModelConfig<Self>) -> (NeuronSet, Vec<usize>) {
        //let d: Array2<f32> = array![EXCITATORY_PARAMS, INHIBITORY_PARAMS];

        let r = IzhikevichParameters::RANGES;

        let dm = m3.mapv(|x| math::ml::sigmoid(x));

        let mut d: Array2<f32> = Array::zeros((2,5));

        // 0: excitatory, 1: inhibitory
        d[[0, 0]] = d[[0, 0]] * (r[0].1 - r[0].0) + r[0].0;
        d[[0, 1]] = d[[0, 1]] * (r[1].1 - r[1].0) + r[1].0;
        d[[0, 2]] = d[[0, 2]] * (r[2].1 - r[2].0) + r[2].0;
        d[[0, 3]] = d[[0, 3]] * (r[3].1 - r[3].0) + r[3].0;
        d[[0, 4]] = 0.0;
        
        d[[1, 0]] = d[[1, 0]] * (r[0].1 - r[0].0) + r[0].0;
        d[[1, 1]] = d[[1, 1]] * (r[1].1 - r[1].0) + r[1].0;
        d[[1, 2]] = d[[1, 2]] * (r[2].1 - r[2].0) + r[2].0;
        d[[1, 3]] = d[[1, 3]] * (r[3].1 - r[3].0) + r[3].0;
        d[[1, 4]] = 1.0;
...
impl Configurable for EDDModel {
    type Config = EDDModelConfig;
}

#[derive(Clone, Debug, Deserialize)]
pub struct EDDModelConfig {
...
impl ConfigSection for EDDModelConfig {
    fn name() -> String {
        "edd_base_model".to_string()
    }
}
